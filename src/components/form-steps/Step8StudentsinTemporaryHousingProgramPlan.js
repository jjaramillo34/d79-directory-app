...
const Step8StudentsinTemporaryHousingProgramPlan = ({ stepData, updateStepData, isActive }) => {
  const [questions] = useState(() => {
    const step = formQuestionsData.steps.find(s => s.key === 'temporaryHousing');
  ...
  const handleInputChange = (questionId, value) => {
    ...
    updateStepData('temporaryHousing', newFormData);
  };
...
export default Step8StudentsinTemporaryHousingProgramPlan;
